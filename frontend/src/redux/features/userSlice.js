...
export const createUser = createAsyncThunk(
  "user/createUser",
  async (
    { email, password, user_name, user_role, sucursal, onClose, reset },
...
const getQuery = (usersCollection, sort = "asc") => {
  return query(
    usersCollection,
    where("active", "==", true),
    orderBy("user_name", sort),
    limit(10)
  );
};
  ...
  async (
    ...
    try {
      ...
      if (!isNextPage && !isPrevPage) {
        const q = getQuery(usersCollection);
        querySnapshot = await getDocs(q);
      }

      if (_users.length === 0 && isPrevPage) {
        const q = getQuery(usersCollection, "desc");
        querySnapshot = await getDocs(q);
        dispatch(resetPage());
      }
...
export const updateUser = createAsyncThunk(
  "event/updateUser",
  async (
    { id, user_name, user_role, sucursal, onClose, reset },
    { dispatch, rejectWithValue }
  ) => {
    try {
